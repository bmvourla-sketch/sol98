...
describe("board-types (Start Ads)", () => {
  ...
  it("board.exe pricing starts at 2 SOL and rises 5% per sale", () => {
    expect(BOARD_FILE_START_PRICE_SOL).toBe(2);
    expect(boardFilePrice(1)).toBeCloseTo(2);
    expect(boardFilePrice(2)).toBeCloseTo(2.1);
    expect(boardFilePrice(3)).toBeCloseTo(2.205);
    expect(nextBoardFilePrice(0)).toBeCloseTo(2);
    expect(nextBoardFilePrice(1)).toBeCloseTo(2.1);
  });
